...
constexpr std::array<int8_t, 128> kBaseTranslation = {
...
Aligner::Aligner(
    ...
    : p{match_score, mismatch_penalty, gap_opening_penalty, gap_extending_penalty} {
  BuildDefaultMatrix();
}
...
void Aligner::SetGapPenalty(const uint8_t opening, const uint8_t extending) {
  p.gap_opening_penalty_   = opening;
  p.gap_extending_penalty_ = extending;
}
...
uint16_t Aligner::AlignImpl(
  ...
  s_align* s_al = ssw_align(
      ...
      static_cast<int>(p.gap_opening_penalty_), static_cast<int>(p.gap_extending_penalty_), flag,
...
void Aligner::BuildDefaultMatrix() {
  ...
  BuildSwScoreMatrix(p.match_score_, p.mismatch_penalty_, score_matrix_);
  translation_matrix_.assign(kBaseTranslation.cbegin(), kBaseTranslation.cend());
}
